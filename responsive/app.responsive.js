...
/* --- Responsive helpers appended by assistant --- */


/* ===== Responsividade & Acessibilidade (adicionadas) ===== */
document.addEventListener('DOMContentLoaded', () => {
  try {
    const sidebar = document.getElementById('sidebar');
    const sidebarToggle = document.getElementById('sidebar-toggle');
    // If the overlay element is missing, create it
    ...
    // Close sidebar when a nav link is clicked (small screens)
    ...
    // Keep aria-hidden in sync on resize and ensure the sidebar is visible on desktop
    const syncSidebarState = () => {
      ...
      if (window.innerWidth > 768) {
        ...
        // on mobile we set aria-hidden based on current open state
        const hidden = !sidebar.classList.contains('open');
    ...
    console.error('Erro no módulo responsivo adicional:', err);
  }
});
